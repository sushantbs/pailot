...
describe('Flight Lifecycle Integration Tests', () => {
  ...
  it('should complete full flight lifecycle: Taxi → Approach transition', async () => {
    ...
    const taxiItems = (await StorageManager.getAllRecallItems()).filter(item =>
      item.phases.includes('Taxi')
    )
    ...
    const approachItems = (await StorageManager.getAllRecallItems()).filter(item =>
      item.phases.includes('Approach')
    )
