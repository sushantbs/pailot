...
export function useRecallManager() {
  ...
  const addRecallItem: (item: Omit<RecallItem, 'id' | 'createdAt' | 'updatedAt'>) => void = useCallback(async (item) => {
    setLoading(true)
    setError(null)
  ...
  const updateRecallItem = useCallback(async (id: number, updates: Partial<RecallItem>) => {
    setLoading(true)
    setError(null)
